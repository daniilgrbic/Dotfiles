...
# This is a modification of d093w1z's polybar-now-playing !
...
from clear_metadata import clear_metadata_string
...
def update_prefix_suffix(player_name="", status=""):
    ...
    play_button = "%%{A:playerctl %s play :}%s%%{A}"        %(player_option,control_chars[1])
    pause_button = "%%{A:playerctl %s pause :}%s%%{A}"      %(player_option,control_chars[2])
